#
#  This code evaluates RAG results on 2 questions (mixture retrieval + rerank). 
#  Fix embedding = 256
#  
#  The demo process: 
#     load data -> split chunks -> load to FAISS -> MIX (vector + BM25) retrieve contexts 
#      -> generate answers on the contexts -> evaluate results 
# 
#  This demo corresponds to L2.18 class nearby 01:49:51.
#
#  The results I ran, using TopK=10, qwen-max model, two questions:
#                     |  256   |  
# | ------------------| ------ | 
...
# |      F1-score     | 0.8537 |
# |    faithfulness   | 0.8125 |
# |  answer relevancy | 0.9365 | 
#
# Compare to the results without reranking, it improves precision a lot!
#
import os
import time
...
from models import qwen_model as llm
from models import embedding_model as embeddings
...
def exec_eval(retriever, questions, ground_truths):
...
# 使用重排序模型
rerank_retriever = get_ali_rerank(top_n=10)
combin_retriever = ContextualCompressionRetriever(
    base_compressor=rerank_retriever, base_retriever=mix_retriever)
mix_evaluate_result = exec_eval(combin_retriever, questions, ground_truths)
