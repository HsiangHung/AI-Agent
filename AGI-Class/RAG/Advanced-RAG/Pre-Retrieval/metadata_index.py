...
# 获得访问 Qwen 大模型
from models import qwen_model as client
...
metadata_field_info = [
    ...
    AttributeInfo(
        name="year",
        description="文章的出版年份，必须是整数 (integer)",
        type="integer",
    ),
    AttributeInfo(
        name="author",
    ...
    AttributeInfo(
        name="rating",
        description="技术价值评估得分，必须是纯数字浮点数 (float),不要使用引号，例如 9.8",
        type="float"
    )
]
...
retriever = SelfQueryRetriever.from_llm(
    ...
    metadata_field_info,
    translator=ChromaTranslator(),   # 关键：确保内部逻辑将查询转换为 Chroma 兼容的格式
...
print(retriever.invoke("我想了解评分在9.0分以上的文章"))
print(retriever.invoke("我想了解评分在9.0分以上的文章,返回其中分数最低的1篇文章"))   #要开启 enable_limit选项
...
query_constructor = prompt | client | output_parser
